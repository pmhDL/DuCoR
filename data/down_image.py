...
parser.add_argument('--remove_pmc', action='store_true', default=True,
                    help='remove pmc articles after image extraction')
...
def download_func(idx):
    ...
    try:
        ...
        if args.remove_pmc and os.path.exists(output_tar_file):
            ...
            shutil.rmtree(
                os.path.join(args.pmc_output_path, str(os.path.basename(sample['pmc_tar_url']))).split('.tar.gz')[
                    0] + '/')
