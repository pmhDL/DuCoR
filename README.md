...
Perform domain alignment using biomedical image-text pairs. This stage trains the visual encoder, projector, and language model adapters to better align visual representations with biomedical language semantics.
...
Conduct medical instruction supervised fine-tuning. The model is initialized from the pretraining checkpoint and uses the same prompt format as the downstream medical VQA tasks.
...
Fine-tune the baseline model on medical VQA datasets using the medical instruction SFT checkpoint as initialization.
...
Initialize DuCoR from the baseline checkpoint and refine it using confidence-weighted pseudo supervision based on baseline-generated pseudo answers.
